...
func NextStringWithNum(num int32) string {
	wid := rand.Int31n(num)
	dId := rand.Int31n(num)
	sn, _ := CreateWorker(int64(wid), int64(dId))
	i := sn.NextId()
...
func NextInt64(ww, did int32) int64 {
	wid := rand.Int31n(ww)
	dId := rand.Int31n(did)
	sn, _ := CreateWorker(int64(wid), int64(dId))
	i := sn.NextId()
...
func GenerateId() int64 {
	worker, _ := CreateWorker(30, 30)
	nextId := worker.NextId()
	return nextId
}

func main() {
	worker, err := CreateWorker(30, 30)
	if err != nil {
		fmt.Println(err)
		return
	}
