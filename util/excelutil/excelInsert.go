...
import (
	...
	//"paas/runingproject/services/excel/export"
	"regexp"
	"runtime/debug"
	"strconv"
	"strings"
...
// 后续使用redis控制
const maxWriteExcelUserCnt = 3

var currentWriteExcelUserCnt = 0
...
func BatchInsertListDataByExcelize(f *excelize.File, SheetName string, rows []map[string]string, titles []map[string]string) (err error) {
	...
	//if export.IsBigDataExport(len(rows)) {
	//	if !export.IsAllowBigDataExport() {
	//		if currentWriteExcelUserCnt > maxWriteExcelUserCnt {
	//			return fmt.Errorf("已有用户正在导出数据，本次单据导出数据量大，服务器忙不过来了，请稍后五分钟再试！")
	//		}
	//		if currentWriteExcelUserCnt < 0 {
	//			currentWriteExcelUserCnt = 0
	//		}
	//		currentWriteExcelUserCnt++
	//		defer func() {
	//			currentWriteExcelUserCnt--
	//		}()
	//
	...
	//文件对象创建的默认第一个Sheel名称就是 Sheet1， 需要修改名称。
	...
	execCpuNum := runtime.NumCPU()/10*4 + 1
		...
		if (i > 0 && i%oneInsertLen == 0) || i == rowsLen-1 {
			...
			go func(rowIndex int, rowsvalueListVSList [][]interface{}) {
				...
				for _, v := range rowsvalueListVSList {
					...
					f.SetSheetRow(SheetName, commutil.AppendStr("A", rowIndex), &v)
				}
